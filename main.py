...
/* 층 선택: 동그라미 아래에 층 이름이 보이도록 세로 배치 */
...
for key, value in {"route": [], "route_cost": 0.0, "route_start": "1층 중앙 현관", "route_end": "컴퓨터실", "map_floor": 1}.items():
...
if swap:
    ...
    st.session_state.map_floor = int(PLACES[end_name]["floor"])
    st.rerun()
...
if st.button("🪄 길 안내 생성", type="primary", use_container_width=True):
    ...
    st.session_state.map_floor = int(PLACES[start_name]["floor"])
...
if route:
    ...
    # 세션 값이 잘못되었거나 경로 생성 직후라면 출발지 층으로 맞춥니다.
    if st.session_state.map_floor not in [1, 2, 3]:
        st.session_state.map_floor = start_floor
