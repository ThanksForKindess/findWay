...
def floor_plan_html(floor: int, path: List[str], start_name: str, end_name: str):
    ...
    if len(points) >= 2:
        ...
        <defs>
            <filter id="arrowShadow" x="-40%" y="-40%" width="180%" height="180%">
                <feDropShadow dx="0" dy="2" stdDeviation="2" flood-color="#0b4fb3" flood-opacity="0.35"/>
            </filter>
        </defs>
        <path id="routePath" d="{path_d}" fill="none" stroke="#1769e8"
              stroke-width="12" stroke-linecap="round" stroke-linejoin="round"/>
        <g class="moving-arrow" filter="url(#arrowShadow)">
            <polygon points="-11,-8 12,0 -11,8 -5,0" fill="#ffffff"/>
        ...
        <g class="moving-arrow" filter="url(#arrowShadow)">
            <polygon points="-11,-8 12,0 -11,8 -5,0" fill="#ffffff"/>
            <animateMotion dur="4.2s" begin="-1.4s" repeatCount="indefinite" rotate="auto">
                <mpath href="#routePath" xlink:href="#routePath"/>
            </animateMotion>
        </g>
        <g class="moving-arrow" filter="url(#arrowShadow)">
            <polygon points="-11,-8 12,0 -11,8 -5,0" fill="#ffffff"/>
            <animateMotion dur="4.2s" begin="-2.8s" repeatCount="indefinite" rotate="auto">
                <mpath href="#routePath" xlink:href="#routePath"/>
            </animateMotion>
        </g>
        '''
    ...
    if start["floor"] == floor:
        markers += f'<circle cx="{start["x"]}" cy="{start["y"]}" r="18" fill="#1769e8" stroke="white" stroke-width="7"/><text class="start-text" x="{start["x"]}" y="{int(start["y"])+43}">출발지</text>'
    if end["floor"] == floor:
        markers += f'<path transform="translate({int(end["x"])-15},{int(end["y"])-45})" d="M15 0C6.7 0 0 6.7 0 15c0 11.5 15 29 15 29s15-17.5 15-29C30 6.7 23.3 0 15 0z" fill="#ef5350" stroke="white" stroke-width="3"/><text class="end-text" x="{end["x"]}" y="{int(end["y"])-55}">도착지</text>'
    ...
    *{{box-sizing:border-box}} body{{margin:0;font-family:Pretendard,"Noto Sans KR",Arial,sans-serif;background:transparent}}
    ...
    .map-area{{background:#fafbfc;padding:8px;overflow:hidden}} svg{{display:block;width:100%;height:auto;margin:0 auto}}
    .room{{fill:#fff;stroke:#c7cbd1;stroke-width:2.5}} .target{{fill:#eef8ef}}
    ...
    .label{{fill:#172033;font-size:25px;font-weight:650;text-anchor:middle;dominant-baseline:middle}} .small{{font-size:18px}} .strong{{font-weight:800}} .muted{{fill:#89919c}} .moving-arrow{{pointer-events:none}}
    .stairs-label{{fill:#68717d;font-size:18px;font-weight:700;text-anchor:middle}} .start-text{{fill:#1769e8;font-size:19px;font-weight:800;text-anchor:middle}} .end-text{{fill:#ef5350;font-size:19px;font-weight:800;text-anchor:middle}}
    </style></head><body><div class="shell"><div class="toolbar"><div><span class="badge">{floor}층</span><span class="title">실내 평면도</span></div><div class="caption">{html_lib.escape(start_name)} → {html_lib.escape(end_name)}</div></div><div class="map-area"><svg viewBox="0 0 900 720" preserveAspectRatio="xMidYMid meet" xmlns:xlink="http://www.w3.org/1999/xlink" style="overflow:hidden"><rect width="900" height="720" rx="24" fill="#fafbfc"/>{rooms_svg(floor)}{route}{markers}</svg></div></div></body></html>'''
...
for key, value in {"route": [], "route_cost": 0.0, "route_start": "1층 중앙 현관", "route_end": "컴퓨터실"}.items():
...
if route:
    ...
    selected_floor = st.radio("지도 층 선택", [1, 2, 3], index=[1,2,3].index(default_floor), horizontal=True, format_func=lambda x: f"{x}층", label_visibility="collapsed")
    html(floor_plan_html(selected_floor, route, route_start, route_end), height=590, scrolling=False)
    ...
    html(floor_plan_html(1, [], st.session_state.route_start, st.session_state.route_end), height=590, scrolling=False)
