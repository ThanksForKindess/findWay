...
def floor_plan_html(floor: int, path: List[str], start_name: str, end_name: str):
    ...
    *{{box-sizing:border-box}} body{{margin:0;font-family:Pretendard,"Noto Sans KR",Arial,sans-serif;background:transparent}}
    ...
    .scroll{{overflow:auto;background:#fafbfc}} svg{{display:block;min-width:690px;width:100%;height:auto}}
    ...
    </style></head><body><div class="shell"><div class="toolbar"><div><span class="badge">{floor}층</span><span class="title">실내 평면도</span></div><div class="caption">{html_lib.escape(start_name)} → {html_lib.escape(end_name)}</div></div><div class="scroll"><svg viewBox="0 0 900 720"><rect width="900" height="720" rx="24" fill="#fafbfc"/>{rooms_svg(floor)}{route}{markers}</svg></div></div></body></html>'''
...
if route:
    ...
    html(floor_plan_html(selected_floor, route, route_start, route_end), height=670, scrolling=True)
    ...
    html(floor_plan_html(1, [], st.session_state.route_start, st.session_state.route_end), height=670, scrolling=True)
