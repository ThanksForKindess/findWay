...
PLACES: Dict[str, Dict[str, object]] = {
    "1층 중앙 현관": {"floor": 1, "node": "entrance", "x": 450, "y": 680},
    "교무실": {"floor": 1, "node": "staff", "x": 178, "y": 560},
    "과학실": {"floor": 1, "node": "science", "x": 120, "y": 200},
    "준비실": {"floor": 1, "node": "prep", "x": 282, "y": 200},
    "음악실": {"floor": 1, "node": "music", "x": 568, "y": 200},
    "컴퓨터실": {"floor": 1, "node": "computer", "x": 760, "y": 200},
    "미술실": {"floor": 1, "node": "art", "x": 180, "y": 310},
    "특별교실 1": {"floor": 1, "node": "special1", "x": 180, "y": 470},
    "도서관": {"floor": 1, "node": "library", "x": 710, "y": 310},
    "특별교실 2": {"floor": 1, "node": "special2", "x": 710, "y": 470},
    "보건실": {"floor": 1, "node": "health", "x": 722, "y": 560},
    "2층 중앙 계단": {"floor": 2, "node": "f2_center", "x": 450, "y": 650},
    "2학년 1반": {"floor": 2, "node": "class201", "x": 137, "y": 300},
    "2학년 2반": {"floor": 2, "node": "class202", "x": 342, "y": 300},
    "영어전용실": {"floor": 2, "node": "english", "x": 737, "y": 300},
    "상담실": {"floor": 2, "node": "counsel", "x": 600, "y": 505},
    "3층 중앙 계단": {"floor": 3, "node": "f3_center", "x": 450, "y": 650},
    "3학년 1반": {"floor": 3, "node": "class301", "x": 137, "y": 300},
    "3학년 2반": {"floor": 3, "node": "class302", "x": 342, "y": 300},
    "AI 융합실": {"floor": 3, "node": "ai_lab", "x": 737, "y": 300},
    "진로활동실": {"floor": 3, "node": "career", "x": 600, "y": 505},
}

NODES: Dict[str, Tuple[int, int, int]] = {
    # 1층 복도 중심선과 교실 문 앞 좌표
    "entrance": (450, 680, 1), "south": (450, 560, 1),
    "center": (450, 365, 1), "north": (450, 200, 1),
    "west_s": (180, 560, 1), "west_c": (180, 365, 1), "west_n": (180, 200, 1),
    "east_s": (710, 560, 1), "east_c": (710, 365, 1), "east_n": (710, 200, 1),
    "science": (120, 200, 1), "prep": (282, 200, 1), "music": (568, 200, 1),
    "computer": (760, 200, 1), "art": (180, 310, 1), "special1": (180, 470, 1),
    "library": (710, 310, 1), "special2": (710, 470, 1),
    "staff": (178, 560, 1), "health": (722, 560, 1), "stairs": (252, 560, 1),

    # 2층
    "f2_center": (450, 650, 2), "f2_vertical": (450, 300, 2),
    "f2_side_top": (600, 300, 2), "f2_side_bottom": (600, 505, 2),
    "class201": (137, 300, 2), "class202": (342, 300, 2),
    "english": (737, 300, 2), "counsel": (600, 505, 2),

    # 3층
    "f3_center": (450, 650, 3), "f3_vertical": (450, 300, 3),
    "f3_side_top": (600, 300, 3), "f3_side_bottom": (600, 505, 3),
    "class301": (137, 300, 3), "class302": (342, 300, 3),
    "ai_lab": (737, 300, 3), "career": (600, 505, 3),
}

EDGES = [
    # 1층: 모든 선분은 복도 중심선을 따라 수평/수직으로만 연결
    ("entrance", "south", 120), ("south", "center", 195), ("center", "north", 165),
    ("south", "west_s", 270), ("center", "west_c", 270), ("north", "west_n", 270),
    ("west_s", "west_c", 195), ("west_c", "west_n", 165),
    ("south", "east_s", 260), ("center", "east_c", 260), ("north", "east_n", 260),
    ("east_s", "east_c", 195), ("east_c", "east_n", 165),
    ("west_n", "science", 60), ("west_n", "prep", 102),
    ("north", "music", 118), ("east_n", "computer", 50),
    ("west_c", "art", 55), ("west_c", "special1", 105),
    ("east_c", "library", 55), ("east_c", "special2", 105),
    ("west_s", "staff", 2), ("east_s", "health", 12),
    ("west_s", "stairs", 72), ("stairs", "f2_center", 210),

    # 2층
    ("f2_center", "f2_vertical", 350),
    ("f2_vertical", "class201", 313), ("f2_vertical", "class202", 108),
    ("f2_vertical", "f2_side_top", 150), ("f2_side_top", "english", 137),
    ("f2_side_top", "f2_side_bottom", 205), ("f2_side_bottom", "counsel", 1),
    ("f2_center", "f3_center", 210),

    # 3층
    ("f3_center", "f3_vertical", 350),
    ("f3_vertical", "class301", 313), ("f3_vertical", "class302", 108),
    ("f3_vertical", "f3_side_top", 150), ("f3_side_top", "ai_lab", 137),
    ("f3_side_top", "f3_side_bottom", 205), ("f3_side_bottom", "career", 1),
]
...
def rooms_svg(floor: int):
    if floor == 1:
        ...
        <path class="corridor" d="M120 200H760"/><path class="corridor" d="M180 200V560H710V200"/><path class="corridor" d="M450 200V680"/>
        '''
    ...
    <path class="corridor" d="M110 300H790"/><path class="corridor" d="M450 215V680"/><path class="corridor side-corridor" d="M600 300V505"/>
    '''
...
def floor_plan_html(floor: int, path: List[str], start_name: str, end_name: str):
    ...
    if len(points) >= 2:
        ...
        <path class="route-flow-fallback" d="{path_d}" fill="none" stroke="rgba(255,255,255,.9)"
              stroke-width="3" stroke-linecap="round" stroke-dasharray="2 28"/>
        <g class="moving-arrows" aria-hidden="true">
            <g class="route-arrow">
                <circle r="13" fill="#0d55c8" stroke="#ffffff" stroke-width="3"/>
                <path d="M-4,-6 L4,0 L-4,6" fill="none" stroke="#ffffff"
            ...
            <g class="route-arrow">
                ...
                <path d="M-4,-6 L4,0 L-4,6" fill="none" stroke="#ffffff"
                      stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/>
                <animateMotion begin="-1.8s" dur="5.4s" repeatCount="indefinite" rotate="auto"
                               keyPoints="0;1" keyTimes="0;1" calcMode="linear">
                    <mpath href="#routePath" xlink:href="#routePath"/>
                </animateMotion>
            </g>
            <g class="route-arrow">
                ...
                <path d="M-4,-6 L4,0 L-4,6" fill="none" stroke="#ffffff"
                      stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"/>
                <animateMotion begin="-3.6s" dur="5.4s" repeatCount="indefinite" rotate="auto"
                               keyPoints="0;1" keyTimes="0;1" calcMode="linear">
                    <mpath href="#routePath" xlink:href="#routePath"/>
                </animateMotion>
            </g>
    ...
    if int(start["floor"]) == floor:
        ...
        label_y = sy - 28 if sy >= 560 else sy - 24
        markers += (
            f'<circle cx="{sx}" cy="{sy}" r="17" fill="#1769e8" stroke="white" stroke-width="7"/>'
            f'<text class="start-text overlay-text" x="{sx}" y="{label_y}">출발지</text>'
        )
    if int(end["floor"]) == floor:
        ex, ey, _ = NODES[end_node]
        marker_y = ey - 42
        label_y = ey - 58
        markers += (
            f'<path transform="translate({ex - 15},{marker_y})" '
            f'd="M15 0C6.7 0 0 6.7 0 15c0 11.5 15 29 15 29s15-17.5 15-29C30 6.7 23.3 0 15 0z" '
            f'fill="#ef5350" stroke="white" stroke-width="3"/>'
            f'<text class="end-text overlay-text" x="{ex}" y="{label_y}">도착지</text>'
        )

    return f'''<!doctype html><html lang="ko" xmlns:xlink="http://www.w3.org/1999/xlink"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
    <style>
    *{{box-sizing:border-box}} body{{margin:0;font-family:Pretendard,"Noto Sans KR",Arial,sans-serif;background:transparent}}
    ...
    .toolbar{{display:flex;justify-content:space-between;align-items:center;padding:12px 16px;border-bottom:1px solid #edf0f3}}
    ...
    .map-wrap{{overflow:hidden;background:#fafbfc;display:flex;justify-content:center;align-items:center;padding:4px 10px 8px}}
    svg{{display:block;width:100%;height:min(59vw,540px);max-height:540px}}
    ...
    .corridor{{fill:none;stroke:#e2e5e9;stroke-width:48;stroke-linecap:round;stroke-linejoin:round}}
    .side-corridor{{stroke-width:32}}
    ...
    .route-arrow{{filter:drop-shadow(0 2px 2px rgba(0,0,0,.28));pointer-events:none}}
    .route-flow-fallback{{animation:routeFlow 1.15s linear infinite;pointer-events:none}}
    @keyframes routeFlow{{to{{stroke-dashoffset:-30}}}}
    @media (max-width:640px){{svg{{height:440px}} .caption{{display:none}}}}
    </style></head><body><div class="shell"><div class="toolbar"><div><span class="badge">{floor}층</span><span class="title">실내 평면도</span></div><div class="caption">{html_lib.escape(start_name)} → {html_lib.escape(end_name)}</div></div><div class="map-wrap"><svg viewBox="0 0 900 720" preserveAspectRatio="xMidYMid meet"><rect width="900" height="720" rx="24" fill="#fafbfc"/>{rooms_svg(floor)}{route}{markers}{overlay_svg(floor)}</svg></div></div></body></html>'''
...
if route:
    ...
    html(floor_plan_html(selected_floor, route, route_start, route_end), height=620, scrolling=False)
